...
function hydrateOperation(operation: Operation): Operation {
  const value = operation.vectorClock as unknown as
    | VectorClock
    | ClockMap
    | { clock: ClockMap };
  ...
  const map =
    typeof value === "object" && value !== null && "clock" in value
      ? value.clock
      : value;
  return { ...operation, vectorClock: VectorClock.from(map as ClockMap) };
}

function hasCompleteCausalPrefix(
...
function hasObservableClockGap(operations: Operation[]): boolean {
  const maximumByDevice = new Map<string, number>();
  for (const operation of operations) {
    const sequence = operation.vectorClock.get(operation.deviceId);
    maximumByDevice.set(
      operation.deviceId,
      Math.max(maximumByDevice.get(operation.deviceId) ?? 0, sequence),
    );
  }
...
function provesDeletedState(operations: Operation[]): boolean {
  let lastLifecycleOperation: Operation | undefined;
  for (const operation of operations) {
...
export async function reconstructHistoricalState(
  ...
  const targetIndex = orderedOperations.findIndex(
    (candidate) => candidate.id === operationId,
  );

  if (targetIndex < 0) return { status: "document_not_found" };
  ...
  if (
    hasObservableClockGap(documentOperations) ||
