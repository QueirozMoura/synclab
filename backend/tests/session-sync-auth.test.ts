...
const config: SessionHttpConfig = {
...
function sessionServiceFor(token: string | null): SessionService {
  return {
    getAuthenticatedUser: async (received?: string) =>
      received === token ? user : null,
  } as SessionService;
}
...
describe("POST /sync session authentication", () => {
  ...
  beforeEach(async () => {
    ...
    registerSyncRoutes(
      ...
      authz,
      new ApiKeyValidator(),
      sessionServiceFor("valid-session"),
      config,
    );
    await app.ready();
  });
