...
import {
  ...
  SyncOperationAdapterError,
} from "../src/application/sync/SyncOperationAdapter.js";
import {
  SyncOperationType,
  type SyncOperation,
...
describe("SyncOperationAdapter", () => {
  ...
  describe("toDomainOperation", () => {
    it("deve rejeitar CREATE_DOCUMENT com erro descritivo", () => {
      ...
      expect(() => SyncOperationAdapter.toDomainOperation(op)).toThrow(
        SyncOperationAdapterError,
      );
      ...
      try {
        ...
        expect((error as SyncOperationAdapterError).syncOperationType).toBe(
          SyncOperationType.CREATE_DOCUMENT,
        );
        expect((error as SyncOperationAdapterError).message).toContain(
          "CREATE_DOCUMENT",
        );
        expect((error as SyncOperationAdapterError).message).toContain(
  ...
  describe("tryAdapt", () => {
    it("deve retornar success: false para CREATE_DOCUMENT", () => {
      ...
      if (!result.success) {
        ...
        expect(result.error.syncOperationType).toBe(
          SyncOperationType.CREATE_DOCUMENT,
        );
      }
    });
  ...
  describe("SyncOperationAdapterError", () => {
    ...
    it("deve ter nome correto", () => {
      ...
      try {
        ...
        expect((error as Error).name).toBe("SyncOperationAdapterError");
      }
    });
  });
});