...
describe("HTTP Sync Routes", () => {
    ...
    const serializeOp = (op) => serializer.serialize(op);
    function injectWithAuth(options) {
        return app.inject({
            ...options,
            headers: { ...options.headers, ...AUTH_HEADER },
        });
    }
    function injectSync(options) {
        return app.inject({
            ...options,
    ...
    describe("POST /sync/push", () => {
        ...
        it("rejeita operação com payload inválido (content deve ser string) - HTTP layer validation", async () => {
            ...
            serialized.payload = { afterId: null, content: 123 };
            const response = await injectWithAuth({
                method: "POST",
                url: "/sync/push",
    ...
    describe("Cenários de sincronização completa", () => {
        ...
        it("POST /sync: VectorClock round-trip com múltiplos dispositivos", async () => {
            ...
            const bodyC = responseC.json();
            expect(bodyC.missingOperations).toHaveLength(2);
            const missingClocks = bodyC.missingOperations.map((op) => op.vectorClock);
            expect(missingClocks.some((vc) => vc["device-A"] === 1 && vc["device-B"] === 1)).toBe(true);
            expect(missingClocks.some((vc) => vc["device-A"] === 1 && !vc["device-B"])).toBe(true);
            const responseA2 = await injectSync({
                method: "POST",
                url: "/sync",
                payload: {
        ...
        it("POST /sync: validação completa do contrato HTTP", async () => {
            ...
            const acceptedTypes = body.acceptedOperations.map((op) => op.type).sort();
        ...
        it("POST /sync: múltiplos documentos isolados", async () => {
            ...
            const body = response.json();
            expect(body.acceptedOperations).toHaveLength(3);
            const docIds = body.acceptedOperations.map((op) => op.documentId).sort();
...
describe("HTTP Sync - Resiliência", () => {
    ...
    const createSyncOperation = (type, payload, overrides = {}) => ({
        id: "op-1",
        documentId: "doc-1",
        deviceId: "device-A",
        type,
