...
describe("auth persistence foundation", () => {
    ...
    const test = (name, fn) => it(name, async () => {
        if (!available)
            return;
        await fn();
    });
    ...
    test("rejeita email duplicado", async () => {
        ...
        await expect(users.create(user("DUPLICATE@example.com"))).rejects.toBeTruthy();
    });
    test("cria e busca auth account vinculada ao usuário", async () => {
        ...
        expect((await accounts.findByProviderAccountId("google", created.providerAccountId))?.id).toBe(created.id);
        expect((await accounts.findByUserIdAndProvider(createdUser.id, "google"))
            ?.userId).toBe(createdUser.id);
    });
    ...
    test("cria, busca, atualiza e revoga sessão", async () => {
        ...
        expect((await sessions.findByTokenHash(created.tokenHash))?.userId).toBe(createdUser.id);
    ...
    test("foreign keys e unicidade de sessão são aplicadas", async () => {
        ...
        await expect(sessions.create({ ...first, id: crypto.randomUUID() })).rejects.toBeTruthy();
    });
});
