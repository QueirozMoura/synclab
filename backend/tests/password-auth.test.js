...
import { PasswordAuthService, EmailAlreadyExistsError, InvalidCredentialsError, } from "@application/auth/PasswordAuthService.js";
...
function repositories(existing = null) {
    const users = {
    ...
    return {
        ...
        accounts,
        sessions,
        service: new PasswordAuthService(users, accounts, sessions),
    };
}
describe("PasswordAuthService", () => {
    it("registra com email normalizado, Argon2id e sessão sem senha em texto puro", async () => {
        ...
        const stored = deps.accounts.create.mock.calls[0][0];
        expect(deps.users.findByEmail).toHaveBeenCalledWith("user@example.com");
        expect(stored.provider).toBe("password");
        expect(stored.passwordHash).toMatch(/^\$argon2id\$/);
        expect(await argon2.verify(stored.passwordHash, "password-123")).toBe(true);
    ...
    it("rejeita duplicação e dados inválidos", async () => {
        await expect(repositories(user).service.register({
            email: "user@example.com",
            password: "password-123",
        })).rejects.toBeInstanceOf(EmailAlreadyExistsError);
        await expect(repositories().service.register({
            email: "invalid",
            password: "password-123",
        })).rejects.toThrow();
        await expect(repositories().service.register({
    ...
    it("faz login normalizado e usa erro genérico para senha ou usuário inválido", async () => {
        ...
        expect((await deps.service.login({
            email: " USER@EXAMPLE.COM ",
            password: "password-123",
        })).token).toBe("opaque-token");
        await expect(deps.service.login({ email: "user@example.com", password: "wrong-pass" })).rejects.toBeInstanceOf(InvalidCredentialsError);
        await expect(repositories(null).service.login({
...
describe("rotas de autenticação por senha", () => {
    it("faz login e emite novo cookie de sessão", async () => {
        ...
        registerAuthRoutes(app, { getAuthenticatedUser: vi.fn(async () => user) }, getSessionHttpConfig({
            NODE_ENV: "test",
        ...
        expect(response.json().user).toEqual(expect.objectContaining({ email: "user@example.com" }));
        await app.close();
    });
...
describe("POST /auth/logout", () => {
    it("revoga a sessão, limpa o cookie e é idempotente", async () => {
        ...
        const app = Fastify();
        await app.register(fastifyCookie);
        registerAuthRoutes(app, { getAuthenticatedUser: vi.fn(async () => null), revokeSession }, getSessionHttpConfig({ NODE_ENV: "test", SESSION_COOKIE_NAME: "test_session", SESSION_TTL_SECONDS: "60" }));
        ...
        expect(response.headers["set-cookie"]).toMatch(/test_session=/);
