...
import { OperationLog, OperationType, } from "../src/domain/operations/index.js";
...
describe("SyncEngine", () => {
    ...
    describe("getOrderedOperations", () => {
        ...
        it("deve ordenar topologicamente quando causalidade e desempate formariam um ciclo no comparador par-a-par", () => {
            ...
            const opA = makeOp("op-a", "doc-1", "device-Z", VectorClock.from({ "device-Z": 1 }));
            const opB = makeOp("op-b", "doc-1", "device-A", VectorClock.from({ "device-Z": 1, "device-A": 1 }));
            const opC = makeOp("op-c", "doc-1", "device-M", VectorClock.from({ "device-M": 1 }));
            const firstReplica = new SyncEngine();
            const secondReplica = new SyncEngine();
            for (const operation of [opA, opB, opC])
                firstReplica.receive(operation);
            for (const operation of [opC, opB, opA])
                secondReplica.receive(operation);
