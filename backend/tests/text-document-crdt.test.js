...
import { createElementId, OperationType } from "../src/domain/operations/index.js";
...
describe("TextDocumentCrdt", () => {
    ...
    it("produz o mesmo texto para o mesmo conjunto em ordens de chegada diferentes", () => {
        ...
        const first = new TextDocumentCrdt("doc-1");
        const second = new TextDocumentCrdt("doc-1");
        first.apply(a);
        first.apply(b);
        second.apply(b);
        second.apply(a);
        expect(first.getState()).toBe("AB");
        expect(second.getState()).toBe("AB");
    });
    ...
    it("rejeita operação de outro documento", () => {
        const crdt = new TextDocumentCrdt("doc-1");
        const foreign = { ...insert("foreign", "device-A", VectorClock.from({ "device-A": 1 }), null, "A"), documentId: "doc-2" };
    ...
    it("mantém determinismo para múltiplos INSERTs concorrentes", () => {
        ...
        const first = new TextDocumentCrdt("doc-1");
        const second = new TextDocumentCrdt("doc-1");
        operations.forEach((operation) => first.apply(operation));
        [...operations].reverse().forEach((operation) => second.apply(operation));
        expect(first.getState()).toBe("ABC");
        expect(second.getState()).toBe("ABC");
    });
    it("usa id da operação como desempate para o mesmo deviceId", () => {
        ...
        second.apply(z);
        second.apply(a);
        expect(first.getState()).toBe("AZ");
        expect(second.getState()).toBe("AZ");
    });
    ...
    it("aplica DELETE de um elemento e omite tombstone do texto", () => {
        ...
        expect(crdt.getVisibleElementIds()).toEqual([createElementId("base", 1)]);
    });
    ...
    it("é idempotente para DELETE duplicado", () => {
        ...
        expect(crdt.apply(deletion)).toBe(true);
        expect(crdt.apply(deletion)).toBe(false);
        expect(crdt.getState()).toBe("");
    });
    it("converge para DELETEs concorrentes", () => {
        ...
        const first = new TextDocumentCrdt("doc-1");
        const second = new TextDocumentCrdt("doc-1");
        [base, deleteA, deleteB].forEach((operation) => first.apply(operation));
        [deleteB, base, deleteA].forEach((operation) => second.apply(operation));
        expect(first.getState()).toBe("");
        expect(second.getState()).toBe("");
    });
    it("converge para INSERT concorrente com DELETE", () => {
        ...
        const first = new TextDocumentCrdt("doc-1");
        const second = new TextDocumentCrdt("doc-1");
        [base, insertion, deletion].forEach((operation) => first.apply(operation));
        [deletion, insertion, base].forEach((operation) => second.apply(operation));
        expect(first.getState()).toBe("B");
        expect(second.getState()).toBe("B");
    });
    ...
    it("mantém IDs de elementos estáveis", () => {
        ...
        expect(id).toBe(createElementId("base", 0));
        expect(crdt.getVisibleElementIds()).toEqual([id, createElementId("suffix", 0)]);
    });
    it("mantém convergência quando causalidade cruza desempate concorrente", () => {
        ...
        operations.forEach((operation) => first.apply(operation));
        [...operations].reverse().forEach((operation) => second.apply(operation));
        expect(first.getState()).toBe("CAB");
        expect(second.getState()).toBe("CAB");
    });
    it("mantém a política atual para conteúdo distinto com o mesmo ID de operação", () => {
        ...
        const b = { ...a, payload: { afterId: null, content: "B" } };
        const first = new TextDocumentCrdt("doc-1");
        const second = new TextDocumentCrdt("doc-1");
        first.apply(a);
        first.apply(b);
    ...
    it("não altera a operação original", () => {
        ...
        const clock = operation.vectorClock.toMap();
        crdt.apply(operation);
        crdt.getState();
        expect(operation.payload).toEqual(payload);
        expect(operation.vectorClock.toMap()).toEqual(clock);
    });
