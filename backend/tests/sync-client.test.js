...
describe("SyncClient", () => {
    ...
    beforeEach(async () => {
        ...
        registerSyncRoutes(app, serverRepository, new InMemoryDocumentOperationRepository(), new InMemoryDocumentSnapshotRepository(), authzRepository, apiKeyValidator);
    ...
    it("push envia operações locais para o servidor", async () => {
        ...
        const client = new SyncClient({ serverUrl, documentId: "doc-1", deviceId: "device-A", apiKey: API_KEY_CLIENT_A_DEVICE_A }, localRepo);
        const ops = [
    ...
    it("reconstrói CRDT após sincronização", async () => {
        ...
        const db = await SqliteFactory.createDatabase();
        const localRepo = new SqliteOperationRepository(db);
        const client = new SyncClient({ serverUrl, documentId: "doc-1", deviceId: "device-C", apiKey: API_KEY_CLIENT_B_DEVICE_C }, localRepo);
    ...
    it("sincronização bidirecional: Device A e Device B convergem", async () => {
        ...
        const clientA = new SyncClient({ serverUrl, documentId: "doc-1", deviceId: "device-A", apiKey: API_KEY_CLIENT_A_DEVICE_A }, repoA);
        const dbB = await SqliteFactory.createDatabase();
        const repoB = new SqliteOperationRepository(dbB);
        const clientB = new SyncClient({ serverUrl, documentId: "doc-1", deviceId: "device-B", apiKey: API_KEY_CLIENT_A_DEVICE_B }, repoB);
    ...
    it("três dispositivos sincronizando", async () => {
        ...
        for (const c of clients) {
            ...
            const client = new SyncClient({ serverUrl, documentId: "doc-1", deviceId: c.id, apiKey: c.apiKey }, repo);
    ...
    it("DELETE sincronizado corretamente", async () => {
        ...
        const db = await SqliteFactory.createDatabase();
        const localRepo = new SqliteOperationRepository(db);
        const client = new SyncClient({ serverUrl, documentId: "doc-1", deviceId: "device-B", apiKey: API_KEY_CLIENT_A_DEVICE_B }, localRepo);
        await client.pullAll();
        const crdt = new TextDocumentCrdt("doc-1");
        const localOps = await localRepo.findByDocumentId("doc-1");
        for (const op of localOps) {
    ...
    it("reconstrução do CRDT depois de reiniciar cliente (persistência SQLite)", async () => {
        ...
        const client1 = new SyncClient({ serverUrl, documentId: "doc-1", deviceId: "device-C", apiKey: API_KEY_CLIENT_B_DEVICE_C }, repo1);
        ...
        const client2 = new SyncClient({ serverUrl, documentId: "doc-1", deviceId: "device-C", apiKey: API_KEY_CLIENT_B_DEVICE_C }, repo2);
