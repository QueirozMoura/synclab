...
import type { Operation } from "../../frontend/src/types/operation";

let activeDeviceId = "device-A";
const persistedOperations = new Map<string, Operation[]>();
...
const API_KEY_BY_DEVICE: Record<string, string> = {
...
function metadataStore() {
  ...
  return {
    ...
    setLastSuccessfulSyncAt: (value: number) => {
      timestamp = value;
    },
  };
}
...
describe("Parte 83 - integração real frontend e backend", () => {
  ...
  beforeAll(async () => {
    ...
    if (!address || typeof address === "string") throw new Error("Failed to obtain test server address");
  ...
  function transportFor(deviceId: string, fetchImpl: typeof fetch = fetch): HttpSyncTransport {
  ...
  function coordinatorFor(manager: OperationManager, deviceId: string, fetchImpl?: typeof fetch): SyncCoordinator {
  ...
  async function newManager(deviceId: string): Promise<OperationManager> {
  ...
  it("mantém pending após falha, permite retry manual e compartilha concorrência", async () => {
    ...
    const fetchWithFailure: typeof fetch = async (url, options) => {
      requests += 1;
  ...
  it("rejeita resposta estruturalmente inválida sem confirmar", async () => {
    ...
    const invalidFetch: typeof fetch = vi.fn().mockResolvedValue(
      new Response(JSON.stringify({ acceptedOperations: {}, missingOperations: [], snapshots: [] }), {
        status: 200,
